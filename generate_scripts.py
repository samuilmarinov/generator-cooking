...
echo -e "${RED}🛑 Stopping Pantry to Plate servers...${NC}"
...
echo -e "${BLUE}🧪 Testing Pantry to Plate...${NC}"
...
if [ "$response" = "200" ]; then
    ...
    recipe_response=$(curl -s -X POST "http://localhost:8001/api/generate-recipe" \
        -H "Content-Type: application/json" \
        -d '{"ingredients":["chicken","rice"],"dietary_preferences":["keto"]}' \
        -w "%{http_code}" 2>/dev/null || echo "000")
    
    if [[ "$recipe_response" == *"200" ]]; then
        echo -e "${GREEN}✅ Recipe generation working${NC}"
    else
        echo -e "${RED}❌ Recipe generation failed${NC}"
    fi
...
echo -e "${BLUE}📚 API Documentation at: http://localhost:8001/docs${NC}"
""",
    "dev.sh": r"""#!/bin/bash
...
echo -e "${BLUE}🔧 Pantry to Plate - Development Menu${NC}"
echo
echo "Choose what to run:"
echo "1. Backend only"
echo "2. Frontend only"
echo "3. Both servers (same as ./start.sh)"
echo "4. Run tests"
echo "5. MongoDB (if installed locally)"
echo "6. Install dependencies"
echo "7. Clean install (remove node_modules and venv)"
echo "8. View logs"
...
case $choice in
    ...
    6)
        ...
        ;;
    7)
        echo -e "${YELLOW}Cleaning install...${NC}"
        rm -rf backend/venv frontend/node_modules
        echo -e "${GREEN}Cleaned. Run ./setup.sh to reinstall.${NC}"
        ;;
    8)
        echo -e "${GREEN}Viewing logs...${NC}"
        echo "Backend logs would be in terminal where you ran the backend."
        echo "Frontend logs would be in terminal where you ran the frontend."
        echo "For production, check system logs or add logging to files."
        ;;
    *)
...
def main():
    ...
    for name, body in SCRIPTS.items():
        p = here / name
        write_file(p, body.strip() + "\n")
        print(f"✔ Wrote {p}")

    print("\nAll scripts created and made executable.\n")
    print("Next:")
    print("  1) Review and replace OPENAI key in setup.sh (DEFAULT_OPENAI_KEY).")
    print("  2) Run: ./setup.sh")
    print("  3) Start MongoDB if local: mongod")
    print("  4) Start app: ./start.sh")
